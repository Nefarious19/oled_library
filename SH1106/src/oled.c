...
static OLED_CommunicationInterface_t oledInterface;
...
static void OLED_writeCommand(uint8_t command)
{
	...
	oledInterface.OLED_InterfaceWriteNBytes(OLED_I2C_ADDR,OLED_commandsBuffer,2);
#elif defined(INTERFACE_SPI)
	oledInterface.OLED_comandMode();
	OLED_commandsBuffer[0] = command;
	oledInterface.OLED_InterfaceWriteNBytes(OLED_commandsBuffer, 1);
#endif
}

static void OLED_writeDoubleCommand(uint8_t commandA, uint8_t commandB)
{
#if defined(INTERFACE_I2C)
	OLED_commandsBuffer[0] = OLED_I2C_COMMAND;
	OLED_commandsBuffer[1] = commandA;
	OLED_commandsBuffer[2] = commandB;
	oledInterface.OLED_InterfaceWriteNBytes(OLED_I2C_ADDR,OLED_commandsBuffer,3);
#elif defined(INTERFACE_SPI)
	oledInterface.OLED_comandMode();
	OLED_commandsBuffer[0] = commandA;
	OLED_commandsBuffer[1] = commandB;
	oledInterface.OLED_InterfaceWriteNBytes(OLED_commandsBuffer, 2);
#endif
}

static void OLED_writeTripleCommand(uint8_t commandA, uint8_t commandB, uint8_t commandC)
{
#if defined(INTERFACE_I2C)
	OLED_commandsBuffer[0] = OLED_I2C_COMMAND;
	OLED_commandsBuffer[1] = commandA;
	...
	oledInterface.OLED_InterfaceWriteNBytes(OLED_I2C_ADDR,OLED_commandsBuffer,4);
#elif defined(INTERFACE_SPI)
	oledInterface.OLED_comandMode();
	OLED_commandsBuffer[0] = commandA;
	OLED_commandsBuffer[1] = commandB;
	OLED_commandsBuffer[2] = commandC;
	oledInterface.OLED_InterfaceWriteNBytes(OLED_commandsBuffer, 3);
#endif
}

void OLED_init(OLED_CommunicationInterface_t * interfaceConfig)
{
	memcpy(&oledInterface, interfaceConfig, sizeof(OLED_CommunicationInterface_t));

#if defined(INTERFACE_SPI)
	oledInterface.OLED_chipSelectLow();
#endif
...
#if defined(INTERFACE_SPI)
	oledInterface.OLED_chipSelectHigh();
#endif
...
void OLED_setContrast (uint8_t contrast)
{
	...
	OLED_writeDoubleCommand(SH1106_COMMAND_SET_CONTRAST_CONTROL_REG, contrast);
...
void OLED_updateScreen(void)
{
...
#endif
		oledInterface.OLED_InterfaceWriteNBytes(OLED_I2C_ADDR, dataBufPtr, OLED_WIDTH + OLED_CONTROL_BYTE_SIZE);
	...
	for(uint8_t i = 0; i < (OLED_HEIGHT>>3) ; i++)
	{
		...
		OLED_writeTripleCommand(SH1106_COMMAND_SET_PAGE_ADRESS | i, SH1106_COMMAND_SET_LOWER_COLUMN_ADDRESS | 0x02, SH1106_COMMAND_SET_HIGHER_COLUMN_ADDRESS );
#endif
		oledInterface.OLED_dataMode();
		oledInterface.OLED_InterfaceWriteNBytes(dataBufPtr, OLED_WIDTH);
